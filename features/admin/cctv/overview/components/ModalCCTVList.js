...
const DetailContent = (props) => {
  const { cctv, station, onOpenConfig } = props;
            ...
            <figure className='h-72 relative overflow-hidden rounded-lg bg-[#101524]'>
              ...
              <section className='bg-gradient-to-r from-black absolute bottom-0 left-0 right-0'>
                <div className='flex justify-between items-center'>
                  ...
                  <div className='block p-3'>
                    <Tag color={item.camera_type === "fixed" ? '#FF4A4A' : '#56E4EE'}>{CAMERA_TYPE[item.camera_type]}</Tag>
                  </div>
                </div>
...
const ModalCCTVList = (props) => {
  const { open, info, onClose, onOpenConfig } = props

  return (
    <Modal
      title={info.station?.station_description || '-'}
      open={open}
      onCancel={() => onClose()}
      footer={false}
      width={'100dvh'}
...
export default React.memo(ModalCCTVList)
