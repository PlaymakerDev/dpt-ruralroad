...
const CardAADT = (props) => {
  const { } = props
  const { loadingPCU } = useWIMContext()
  const data = useAppSelector(state => state.dashboard.pcu)

  return (
    <div className='aadt-container p-5 rounded-lg'>
      <div className='flex gap-5 flex-wrap'>
        <TruckIcon width={55} height={48} color='#FFFFFF' />
        <div className='flex flex-col flex-grow justify-between flex-wrap'>
          <div className='flex flex-col'>
            <h1 className='text-[clamp(1px, 4vw, 15px)]'>AADT (วันนี้)</h1>
            <p className='text-[clamp(1px, 4vw, 15px)]'><span className='text-green-500 text-2xl font-bold'>{stf(data.data.aadt).normal() || 0}</span> คัน/วัน</p>
          </div>
        </div>
      </div>
    </div>)
...
export default React.memo(CardAADT)
