...
const FormSearchExclusive = () => {
  ...
  const handlerSubmit = useCallback(async (values) => {
    ...
    const requestUrl = `${config.hostBackend}/api/v1/reports/exclusive?year_type=be_year&plan_year=${plan_year}&file_type=pdf`;
    setLoading(true);
...
export default React.memo(FormSearchExclusive);
