...
const DetailContent = (props) => {
  const { cctv, setOpen } = props;

  const renderImageCard = useMemo(() => {
    let arrList = [...cctv]?.filter(item => item.camera_status === 'Online')?.sort(() => Math.random() - 0.5).slice(0, 4)

    // CHECK IF ARRLIST HAVE A LENGTH
    if (!arrList.length) {
      return (
        <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
          <figure className='h-44 flex justify-center items-center'>
            <Empty
