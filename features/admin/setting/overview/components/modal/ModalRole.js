...
const Content = (props) => {
  ...
  const handlerSubmit = useCallback(async (values) => {
    if (type === 'edit') {
      const { success } = await funcPut(`/api/v1/masters/user_position/${data?.pid}`, { p_name: values?.role, p_id: data?.pid }, undefined, false)
      if (success) {
        funcGet({})
      ...
      const { success } = await funcPost(`/api/v1/masters/user_position`, { p_name: values?.role }, undefined, false)
      if (success) {
        funcGet({})
        setOpen({ open: false, data: null, type: null })
  ...
  return (
    <Form form={form} handlerSubmit={[buildValue, handlerSubmit]}>
      ...
      <button type="submit" hidden ref={ref_btn}></button>
    </Form>
  );
};
...
const ConfirmDelete = (props) => {
  ...
  const handlerDelete = useCallback(async () => {
    const { success } = await funcDelete(`/api/v1/masters/user_position/${data?.pid}`)
    if (success) {
      funcGet({})
      setOpen({ open: false, data: null, type: null })
    }
...
const ModalRole = (props) => {
  const { open, setOpen, type, data, funcGet } = props;
  const [funcPut, loadingPut] = usePutAPI();
  const [funcPost, loadingPost] = usePostAPI();
