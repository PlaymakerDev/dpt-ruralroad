...
import { API_PUT_DATA_SUCCESS, ERROR_MESSAGE_INTERNAL_SERVER_ERROR } from '@/utils/constant'
...
const usePutAPI = <Data>(loadType: 'overlay' | 'none' = 'overlay', reducer?: ApiReducerOpt) => {
  ...
  const func = useCallback(async (path: string, body: any, params: Record<string, any>, showDefaultMessage: boolean = true) => {
    ...
    try {
      ...
      const { data } = await API.put<ApiResponse<Data>>(path, body, { params })
      ...
      if (showDefaultMessage) {
        if (data?.response?.message) {
          ...
          message.success(API_PUT_DATA_SUCCESS)
        }
      }
